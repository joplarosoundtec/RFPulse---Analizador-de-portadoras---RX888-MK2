# RfPulse 0.12.0-alpha

Duodécima alpha: estabiliza el cartel del pico principal y corrige la carga
del logo cuando el ejecutable se lanza con un directorio de trabajo
distinto al suyo propio.

## Novedades de esta alpha

- **Cartel del pico principal estable**: aunque la búsqueda de pico ya usa
  la traza promediada (desde la 0.9.0-alpha), el cartel (la caja de texto
  con la frecuencia/dBFS) podía seguir "dando botes" en pantalla por
  pequeñas oscilaciones del pico entre bins casi idénticos de una trama a
  otra — reportado por el usuario como difícil de leer. Ahora la posición y
  el valor mostrados en el cartel se suavizan por separado del marcador
  real (que sigue exactamente el pico sin tocar, igual que la traza y el
  resto del gráfico): pequeños cambios se deslizan suavemente, y un cambio
  grande (un retonizado real, u otra señal que pasa a ser la más fuerte) se
  salta directo, sin perseguirlo a cámara lenta.
- **Logo de la pantalla de carga, siempre localizado**: `logo.png` se
  cargaba desde una ruta relativa al directorio de trabajo ACTUAL del
  proceso, no a la carpeta donde está el propio `.exe` — coinciden al
  lanzarlo con doble clic desde el Explorador, pero no en otros contextos
  habituales (p.ej. el depurador de Visual Studio, cuyo directorio de
  trabajo por defecto suele ser el del proyecto, no el de salida).
  Reportado por el usuario: el logo aparecía en el precompilado
  distribuido pero no en su propia compilación. Corregido resolviendo la
  ruta junto al propio ejecutable (`GetModuleFileNameW`), sin depender de
  cómo ni desde dónde se lance.

## Bugs corregidos en esta alpha

- El cartel del pico principal del espectro rebotaba visiblemente en
  pantalla siguiendo pequeñas oscilaciones del pico entre tramas, incluso
  con la búsqueda ya sobre la traza promediada — reportado y confirmado
  por el usuario con hardware real. Corregido con un suavizado dedicado
  solo para el cartel, sin tocar el marcador ni el resto del gráfico.
- El logo de la pantalla de carga no aparecía al compilar y ejecutar el
  proyecto en algunos entornos (p.ej. desde el depurador de Visual
  Studio), aunque el archivo `logo.png` estuviera presente junto al
  ejecutable — causado por resolver la ruta relativa al directorio de
  trabajo del proceso en vez de a la carpeta del propio `.exe`.

---

# RfPulse 0.11.0-alpha

Undécima alpha: corrige un arrastre de datos viejos en el promedio del
espectro tras cambiar de frecuencia.

## Novedades de esta alpha

- **Reset completo del promedio al cambiar de frecuencia/span/dispositivo**:
  `resetHolds()` limpiaba max-hold y min-hold, pero no el promedio
  exponencial (una limitación conocida, ya comentada en el código en
  alphas anteriores, pero nunca cerrada). El usuario reportó que al
  cambiar de frecuencia los valores anteriores se quedaban visibles varios
  segundos antes de converger a los nuevos. Ahora `resetHolds()` también
  limpia el promedio y hace que la siguiente trama lo fije directamente
  (en vez de mezclarlo con el valor anterior), así que un cambio de
  frecuencia, span o dispositivo deja el espectro completamente limpio de
  inmediato, no solo max/min hold.

## Bugs corregidos en esta alpha

- El promedio del espectro seguía mostrando datos de la frecuencia
  anterior durante varios segundos después de cualquier retonización
  (cambio de frecuencia central, span, o dispositivo), mezclándolos
  gradualmente con los nuevos vía el promediado exponencial en vez de
  arrancar limpio. Reportado por el usuario probando sin hardware
  conectado (generador sintético). Corregido extendiendo
  `SpectrumProcessor::resetHolds()` para también limpiar el promedio.
