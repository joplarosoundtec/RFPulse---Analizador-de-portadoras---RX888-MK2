...
void Application::applyCenterFrequency(double frequencyHz)
{
    ...
    if (spectrumProcessor_) {
        ...
        // Promedio/max-hold/min-hold acumulan potencia por INDICE de bin,
        // que tras un cambio de centro pasa a representar una frecuencia
        // distinta a la de antes -- sin resetear, seguirian mostrando
        // datos del espectro anterior mal etiquetados sobre el eje nuevo
        // (reportado por el usuario: los valores viejos se quedaban varios
        // segundos tras cambiar de frecuencia), igual que ya se hacia al
        // cambiar el span (ver applySpanHz).
        spectrumProcessor_->resetHolds();
    }
}
