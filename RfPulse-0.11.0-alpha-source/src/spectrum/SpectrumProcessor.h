...
public:
    ...
    // Limpia max hold, min hold Y el promedio exponencial, y hace que la
    // siguiente trama fije el promedio directamente (en vez de mezclarlo
    // con el valor anterior) -- pensado para llamarse cada vez que cambia
    // lo que representa un indice de bin (centro, span, o tamaño de FFT),
    // para que ninguna traza acumulada seguir arrastrando datos de la
    // asignacion de frecuencias anterior.
    void resetHolds();
