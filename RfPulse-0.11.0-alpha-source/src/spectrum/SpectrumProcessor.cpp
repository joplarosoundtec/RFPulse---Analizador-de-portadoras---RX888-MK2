...
void SpectrumProcessor::resetHolds()
{
    ...
    // El promedio exponencial tambien queda obsoleto cuando cambia lo que
    // representa cada bin (frecuencia central, span, o tamaño de FFT): sin
    // esto, tras un cambio el promedio seguia mezclando datos de la
    // asignacion de frecuencias VIEJA con los de la nueva, arrastrando el
    // valor anterior durante varios segundos en vez de arrancar de cero
    // (reportado por el usuario). firstFrame_ = true hace que el proximo
    // submit() FIJE el promedio directamente al valor de esa trama (ver la
    // rama firstFrame_ en submit()) en vez de mezclarlo con el 0.0f puesto
    // aqui, que si no se leeria como un pico negativo real por un instante.
    std::fill(averagePowerLinear_.begin(), averagePowerLinear_.end(), 0.0f);
    firstFrame_ = true;
}
