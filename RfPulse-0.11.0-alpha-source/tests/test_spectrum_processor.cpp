...
TEST(SpectrumProcessor, ResetHoldsClearsMaxMinHoldAndAverage)
{
    SpectrumProcessor processor(1);
    processor.setMaxHoldEnabled(true);
    processor.setMinHoldEnabled(true);
    // Por defecto AveragingMode::Exponential (ver el valor por defecto de
    // averagingMode_ en SpectrumProcessor.h): sin resetHolds(), el 1.0f de
    // antes del reset seguiria influyendo en el promedio durante varias
    // tramas mas via el mezclado exponencial, en vez de partir limpio.
    std::vector<float> dbScratch;
    processor.submit(MakeResult({1.0f}, dbScratch), 0.0, 1.0);
    ...
    // El promedio tampoco deberia recordar el 1.0f (0 dB) de antes del
    // reset: firstFrame_ vuelve a true, asi que esta trama FIJA el
    // promedio directamente en vez de mezclarlo con el valor anterior.
    EXPECT_NEAR(frame.averageDb[0], -10.0f, 0.1f);
}
